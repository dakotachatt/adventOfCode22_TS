...
export async function day5b(dataPath?: string) {
  ...
  for(let i = (stackBottomIndex + 3); i < data.length; i++ ) {
    ...
    const crates = crateConfiguration[sourceStack - 1].splice(crateConfiguration[sourceStack - 1].length - numCratesToMove);

    console.log('crates', crates);
    console.log('crate config after', crateConfiguration);

    crateConfiguration[destinationStack - 1].push(...crates);
  }
